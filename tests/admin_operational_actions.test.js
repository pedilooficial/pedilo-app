...
test("admin UI executes only backend-provided operational actions with confirmation", () => {
  ...
  assert.match(detailScreen, /primaryAction = allowedActions\.firstOrNull\(\)/);
  assert.match(source, /AdminSecondaryActionDock/);
  assert.match(detailScreen, /AdminSecondaryActionDock/);
  assert.match(detailScreen, /AdminPrimaryActionPanel/);
  ...
  assert.doesNotMatch(detailScreen, /Más acciones|Motivo operativo|Acción o lectura|Datos secundarios/);
